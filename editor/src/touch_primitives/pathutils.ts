		...
		if (pi[0] == "M") {
			...
			for (var j = 0, jj = path2.length; j < jj; j++) {
				...
				if (pj[0] == "M") {
					...
					if (justCount) {
						res = (res as number) + (intr as number);
					} else {
						for (var k = 0, kk = (intr as any[]).length; k < kk; k++) {
							intr[k].segment1 = i;
							intr[k].segment2 = j;
							intr[k].bez1 = bez1;
							intr[k].bez2 = bez2;
						}
						res = (res as any[]).concat(intr);
					}
				}
			}
		}
...
function parsePathString (pathString) {
    ...
    if (!data.length) {
        String(pathString).replace( /([a-z])[\s,]*((-?\d*\.?\d*(?:e[\-+]?\d+)?[\s]*,?[\s]*)+)/ig, (a, b, c) => {
            var params = [],
                name = b.toLowerCase();
	...
	for (var r, pa, i = start, ii = pathArray.length; i < ii; i++) {
		...
		if (pa0 != pa0.toUpperCase()) {
			...
			switch (r[0]) {
				...
				case "O":
					res.pop();
					dots = ellipsePath(x, y, pa[1], pa[2]) as any;
					dots.push(dots[0]);
					res = res.concat(dots);
					break;
				case "U":
