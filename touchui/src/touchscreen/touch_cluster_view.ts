...
import { each, indexOf, extend } from 'lodash';
import 'jqueryui';
...
(function ($) {
    ...
    $.widget("interstate.touch_cluster", {
        options: {
            ...
            fills: touchClusterColors.fills,
            strokes: touchClusterColors.strokes,
        },
        _create: function () {
            this._super();
            this.clusters = [];
        ...
        _getDrawListener: function(cluster, clusterAttributes) {
            var attributes = extend({}, this.option("defaultClusterAttributes"), clusterAttributes),
                fills = this.option("fills"),
                strokes = this.option("strokes"),
                fillIndex = cluster.id()%fills.length,
                fill = fills[fillIndex],
