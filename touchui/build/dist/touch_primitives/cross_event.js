...
var CrossEvent = /** @class */ (function (_super) {
    __extends(CrossEvent, _super);
    function CrossEvent(options) {
        var _this = _super.call(this) || this;
        _this.options = options;
        _this._onCross = function (event) {
            _this.emit('cross', event);
        };
        return _this;
    }
    ;
    CrossEvent.prototype.getPath = function () {
        return this.options.path;
    };
    ;
    CrossEvent.prototype.destroy = function () {
        removeCrossingPathListener(this.getPath());
    };
    ;
    CrossEvent.prototype.getCluster = function () {
        return this.options.cluster;
    };
    ;
    CrossEvent.prototype.onCross = function () {
...
ist.CrossEventAttachment = ist.register_attachment("cross_event_attachment", {
    ...
    */
var lowerCase = String.prototype.toLowerCase, upperCase = String.prototype.toUpperCase, isnan = { "NaN": 1, "Infinity": 1, "-Infinity": 1 }, pathCommand = /([achlmrqstvz])[\x09\x0a\x0b\x0c\x0d\x20\xa0\u1680\u180e\u2000\u2001\u2002\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200a\u202f\u205f\u3000\u2028\u2029,]*((-?\d*\.?\d*(?:e[\-+]?\d+)?[\x09\x0a\x0b\x0c\x0d\x20\xa0\u1680\u180e\u2000\u2001\u2002\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200a\u202f\u205f\u3000\u2028\u2029]*,?[\x09\x0a\x0b\x0c\x0d\x20\xa0\u1680\u180e\u2000\u2001\u2002\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200a\u202f\u205f\u3000\u2028\u2029]*)+)/ig, pathValues = /(-?\d*\.?\d*(?:e[\-+]?\d+)?)[\x09\x0a\x0b\x0c\x0d\x20\xa0\u1680\u180e\u2000\u2001\u2002\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200a\u202f\u205f\u3000\u2028\u2029]*,?[\x09\x0a\x0b\x0c\x0d\x20\xa0\u1680\u180e\u2000\u2001\u2002\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200a\u202f\u205f\u3000\u2028\u2029]*/ig, l2c = function (x1, y1, x2, y2) {
    ...
    var _120 = Math.PI * 120 / 180, rad = Math.PI / 180 * (+angle || 0), res = [], xy, rotate = cacher(function (x, y, rad) {
    ...
    if (!recursive) {
        ...
        x2 = xy.x;
        y2 = xy.y;
        var cos = Math.cos(Math.PI / 180 * angle), sin = Math.sin(Math.PI / 180 * angle), x = (x1 - x2) / 2, y = (y1 - y2) / 2;
        var h = (x * x) / (rx * rx) +
            (y * y) / (ry * ry);
        if (h > 1) {
            h = Math.sqrt(h);
    ...
    var l1 = bezlen.apply(0, bez1), l2 = bezlen.apply(0, bez2), n1 = Math.max(~~(l1 / 5), 1), n2 = Math.max(~~(l2 / 5), 1), dots1 = [], dots2 = [], xy = {}, res = justBool ? false : [], p;
    ...
    for (i = 0; i < n1; i++) {
        for (var j = 0; j < n2; j++) {
            var di = dots1[i], di1 = dots1[i + 1], dj = dots2[j], dj1 = dots2[j + 1], ci = Math.abs(di1.x - di.x) < 0.001 ? "y" : "x", cj = Math.abs(dj1.x - dj.x) < 0.001 ? "y" : "x", is = intersect(di.x, di.y, di1.x, di1.y, dj.x, dj.y, dj1.x, dj1.y);
            if (is) {
                if (xy[is.x.toFixed(4)] == is.y.toFixed(4)) {
                    continue;
                }
                xy[is.x.toFixed(4)] = is.y.toFixed(4);
                var t1 = di.t + Math.abs((is[ci] - di[ci]) / (di1[ci] - di[ci])) * (di1.t - di.t), t2 = dj.t + Math.abs((is[cj] - dj[cj]) / (dj1[cj] - dj[cj])) * (dj1.t - dj.t);
...
}, has = "hasOwnProperty", p2s = /,?([a-z]),?/gi, clone = function (obj) {
    ...
    var res = [], x = 0, y = 0, mx = 0, my = 0, start = 0, dots;
    if (pathArray[0][0] == "M") {
        x = +pathArray[0][1];
        y = +pathArray[0][2];
        mx = x;
    ...
    return p2 ? [p, p2] : p;
}, null, pathClone), interPathHelper = function (path1, path2, justBool) {
    path1 = _path2curve(path1);
    // path2 = _path2curve(path2);
    var x1, y1, x2, y2, x1m, y1m, x2m, y2m, bez1, bez2, res = justBool ? false : [];
...
var touches, latest_touches, last_time, crossingPaths = [], set_touches = function (to_touches) {
    var time = (new Date()).getTime(), i = 0, len = crossingPaths.length, crossingPath, path, touchCluster, cpPath, inter, oldX, oldY, newX, newY;
    while (i < len) {
        ...
        if (path && touchCluster) {
            ...
            if (oldX && oldY && newX && newY) {
                ...
                if (inter) {
                    var d = Math.sqrt(Math.pow(newX - oldX, 2) + Math.pow(newY - oldY, 2)), v = d / (time - crossingPath._time);
...
}, timeout_id, interval_listener = function () {
    timeout_id = false;
    var new_touches = map(latest_touches, function (t) {
...
}, crossing_path_listener_id = 1, addCrossingPathListener = function (touchCluster, path, callback, context) {
...
//# sourceMappingURL=cross_event.js.map