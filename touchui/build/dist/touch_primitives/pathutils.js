...
function interHelper(bez1, bez2, justCount) {
    ...
    for (i = 0; i < n1; i++) {
        for (var j = 0; j < n2; j++) {
            var di = dots1[i], di1 = dots1[i + 1], dj = dots2[j], dj1 = dots2[j + 1], ci = Math.abs(di1.x - di.x) < .001 ? "y" : "x", cj = Math.abs(dj1.x - dj.x) < .001 ? "y" : "x", is = intersect(di.x, di.y, di1.x, di1.y, dj.x, dj.y, dj1.x, dj1.y);
            if (is) {
...
function pathToString() {
    return this.join(",").replace(/,?([a-z]),?/gi, "$1");
}
...
function catmullRom2bezier(crp, z) {
    var d = [];
    for (var i = 0, iLen = crp.length; iLen - 2 * (z ? 0 : 1) > i; i += 2) {
        var p = [
            { x: +crp[i - 2], y: +crp[i - 1] },
            { x: +crp[i], y: +crp[i + 1] },
            { x: +crp[i + 2], y: +crp[i + 3] },
...
function a2c(x1, y1, rx, ry, angle, large_arc_flag, sweep_flag, x2, y2, recursive) {
    ...
    var _120 = Math.PI * 120 / 180, rad = Math.PI / 180 * (+angle || 0), res = [], xy, rotate = memoize(function (x, y, rad) {
    ...
    if (!recursive) {
        ...
        var rx2 = rx * rx, ry2 = ry * ry, k = (large_arc_flag == sweep_flag ? -1 : 1) *
            Math.sqrt(Math.abs((rx2 * ry2 - rx2 * y * y - ry2 * x * x) / (rx2 * y * y + ry2 * x * x))), cx = k * rx * y / ry + (x1 + x2) / 2, cy = k * -ry * x / rx + (y1 + y2) / 2, f1 = Math.asin(parseFloat(((y1 - cy) / ry).toFixed(9))), f2 = Math.asin(parseFloat(((y2 - cy) / ry).toFixed(9)));
...
function pathBBox(path) {
    ...
    var xmin = Math.min.apply(0, X), ymin = Math.min.apply(0, Y), xmax = Math.max.apply(0, X), ymax = Math.max.apply(0, Y), bb = box(xmin, ymin, xmax - xmin, ymax - ymin);
    pth.bbox = clone(bb);
    return bb;
}
